...
public class CubeHBaseEndpointRPC extends CubeHBaseRPC {
    ...
    public IGTScanner getGTScanner(final List<GTScanRequest> scanRequests) throws IOException {
        ...
        final String responseDumpPath = BackdoorToggles.getStorageResultDump();
                        ...
                        for (Map.Entry<byte[], CubeVisitProtos.CubeVisitResponse> result : results.entrySet()) {
                            ...
                            if (result.getValue().getStats().getNormalComplete() != 1) {
                                ...
                                try {
                                    ...
                                    if (responseDumpPath != null) {
                                        try {
                                            String path = responseDumpPath + "/" + order;
                                            FileUtils.writeByteArrayToFile(new File(path), compressed);
                                            logger.info("write to file for part " + order + " finished to " + path);
                                        } catch (Exception e) {
